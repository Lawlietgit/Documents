...
Define $g=|\bg|$, $r=|\br|$, with $\bg$ and $\br$ vectors in 3 dimensional space, $\bg$-space and $\br$-space respectively.\\
...
Similarly we can write  $\ol n^{(\rP\ 2,M,\rEES)}(\bg)$ as
...
&= \sum_J \sum_{\hspJn}^\pzn \phi_J^{(\rcore,\rxc,\rB)}(\hs')\delta_{\hs,\hs'}  \hsinnEES \\
...
\item For every atom $J$, calculate $\phi_J^{(\rP\ 2,\rxc,\rA,\rEES)}(\hs)$ by summing over $f$ as in Eq.\eqref{eq:dExccoredRwhere}. $\sim$ $N N_f \sim N$.
...
&\times \phi_{JK}^{(\rcore,\rcore,\rH,\rshort,\rself)}(\br_f,\br_{f'})\\
\end{split}
\end{equation}
...
\phi_{JK}^{(\rcore,\rcore,\rH,\rshort,\rself)}(\br_f,\br_{f'})&=\frac{\rerfc(\alpha|\br_f-\br_{f'}|)-\rerfc(\delta \Rpc^{-1}|\br_f-\br_{f'}|)}{|\br_f-\br_{f'}|}\\
\end{split}
\end{equation}


The nearest-neighbor term and the self term cost $N N_f^2N_\rNN \sim N$ and $N N_f^2 \sim N$ respectively.
...
&= \sum_{f,f'} w_f w_{f'} \sum_J \frac{\p n_J^{(\rP\ l,\rEES)}(\br_f) }{\p \psigsc}n_J^{(\rP,l',\rEES)}(\br_{f}')\phi_{JK}^{(\rcore,\rcore,\rH,\rshort,\rself)}(\br_f,\br_{f'}) \\
&+ \sum_{f,f'} w_f w_{f'} \sum_J \frac{\p n_J^{(\rP,l',\rEES)}(\br_{f'}) }{\p \psigsc}n_J^{(\rP\ l,\rEES)}(\br_{f})\phi_{JK}^{(\rcore,\rcore,\rH,\rshort,\rself)}(\br_f,\br_{f'}) \\
&= \sum_{f,f'} w_f w_{f'} \sum_J \frac{\p n_J^{(\rP\ l,\rEES)}(\br_f) }{\p \psigsc}n_J^{(\rP,l',\rEES)}(\br_{f}')\phi_{JK}^{(\rcore,\rcore,\rH,\rshort,\rself)}(\br_f,\br_{f'}) \\
&+ \sum_{f,f'} w_f w_{f'} \sum_J \frac{\p n_J^{(\rP,l',\rEES)}(\br_f) }{\p \psigsc}n_J^{(\rP\ l,\rEES)}(\br_{f}')\phi_{JK}^{(\rcore,\rcore,\rH,\rshort,\rself)}(\br_f,\br_{f'}) \\
...
\begin{split}
\phi_{JK}^{(\rcore,\rcore,\rH,\rshort,\rself)}(\br_f,\br_{f'})&=\frac{\rerfc(\alpha|\br_f-\br_{f'}|)-\rerfc(\delta \Rpc^{-1}|\br_f-\br_{f'}|)}{|\br_f-\br_{f'}|}\\
\phi_J^{(\rP\ l,\rcore,\rH,\rshort,\rself,\rEES)}(\br_f) & = \frac{e^2}{2}\sum_{f'} w_f' n_J^{(\rP\ l,\rEES)}(\br_{f}')\phi_{JK}^{(\rcore,\rcore,\rH,\rshort,\rself)}(\br_f,\br_{f'}) \\
...
&\times \frac{\rerfc(\alpha|\br_f-\br_{f'}|)-\rerfc(\delta \Rpc^{-1}|\br_f-\br_{f'}|)}{|\br_f-\br_{f'}|}\\
&= \sum_{f,f'} w_f w_{f'} \frac{\p n_J^{(\rP\ l,\rEES)}(\br_f) }{\p \RJb}n_J^{(\rP,l',\rEES)}(\br_{f}')\phi_{JK}^{(\rcore,\rcore,\rH,\rshort,\rself)}(\br_f,\br_{f'}) \\
&+ \sum_{f,f'} w_f w_{f'} \frac{\p n_J^{(\rP,l',\rEES)}(\br_{f'}) }{\p \RJb}n_J^{(\rP\ l,\rEES)}(\br_{f})\phi_{JK}^{(\rcore,\rcore,\rH,\rshort,\rself)}(\br_f,\br_{f'}) \\
...
&+ \frac{e^2}{2V}  \sum_{l=1}^2 \sum_{\bg \neq 0}^{\Gc} \ol \chi^{(\rlong)} (\bg) \frac{\p \ol{n}^{(\rP\ l,M,\rEES)}(\bg)}{\p \RJb}\ol{n}^{(\rtot,\rEES)*}(\bg) \\
&= \frac{e^2}{V}  \sum_{l=1}^2 \sum_{\bg \neq 0}^{\Gc} \ol \chi^{(\rlong)} (\bg) \frac{\p \ol{n}^{(\rP\ l,M,\rEES)}(\bg)}{\p \RJb}\ol{n}^{(\rtot,\rEES)*}(\bg) \\
&=\sum_{\hsJp}^{\pzp} \left(\Psi_J^{(\rS,Z,D)}(\hat \bs)+ \rcc\right) F_{J\beta}^{(R,\rP\ 1,\rtot,\rH,\rlong,1,a,\rEES)}(\hs) \\
...
\phi_J^{(\rP\ 1,\rtot,\rH,\rlong,\rA,\rEES)}(\hs) &= \sum_f w_f \Delta p(\br_f) \phi_J^{(\rcore,\rtot,\rH,\rlong,\chi,D,\rA,\rEES)}(\br_f)  \sum_{\bk} \Mp(\bu_{Jf}-\hat \bs) \delta_{\hat \bs,\bl_{Jf}-\bk} \hsinJp \\
F_{J\beta}^{(R,\rS,Z,\Psi,\rEES)}(\hs) &= \sum_I \frac{\p Z_{IJ}^{(\rS,\rEES)}}{\p \RJb}\Psi_I^{(\rS,\rD)*}(\hat \bs) + \rcc \hsinJp\\
F_{J\beta}^{(R,\rP\ 1,\rtot,\rH,\rlong,1,a,\rEES)}(\hs) &= \sum_f w_f \Delta p(\br_f) \phi_J^{(\rcore,\rtot,\rH,\rlong,\chi,D,\rA,\rEES)}(\br_f) \sum_{\bk} \frac{\p \Mp(\bu_{Jf}-\hat \bs)}{\p \RJb} \delta_{\hat \bs,\bl_{Jf}-\bk} \hsinJp \\
...
&=&\sum_J Q_J \sum_{\bk}^{p^3} \Mp(\bu_{J}-\hat \bs)\delta_{\hat \bs,\bl_J-\bk} \hsinpEES\\
\label{gr:SnEES}
S^{(\rCo,n,\rEES)}(\hat \bs)
&=&\sum_J Q_J \sum_{\bk}^{p^3} \Mn(\bu_{J}-\hat \bs)\delta_{\hat \bs,\bl_J-\bk} \hsinnEES
\end{eqnarray}
%%%%%%%%%%%%%%%%%%%%%%%%
...
&=& \frac{e^2}{2}\sum_{f,f'} w_f w_{f'} \sum_{l,l'=1}^2\sum_J n_J^{(\rP\ l,\rEES)}(\br_f) n_J^{(\rP,l',\rEES)}(\br_{f'})\phi_{JK}^{(\rcore,\rcore,\rH,\rshort,\rself)}(\br_f,\br_{f'})\nonumber\\
\\
...
\phi_J^{(\rP\ 1,\rxc,\rA,\rEES)}(\hs) &=& \sum_f w_f \Delta p(\br_f) \mu_{\rxc}\left(n_J^{(\rtot,\rEES)}(\br_f)\right)  \sum_{\bk} \Mp(\bu_{Jf}-\hat \bs)\delta_{\hat \bs,\bl_{Jf}-\bk} \hsinJp\nonumber\\
&\ &\\
\phi_J^{(\rP\ 2,\rxc,\rA,\rEES)}(\hat \bs) &=& \sum_f w_f \Delta p^2(\br_f) \mu_{\rxc}\left(n_J^{(\rtot,\rEES)}(\br_f)\right)  \\
...
\begin{eqnarray}
Z_{IJ}^{(W,\rP\ 1,\rlo,\rshort,a,\rEES)} &=& \sum_{\hsJp}^{\pzp} \Psi_{I}^{(\rS,D)}(\hat \bs) \phi_J^{(\rP\ 1,\rlo,\rshort,\rEES)}(\hs)\\
F_I^{(W,\rP\ 1,\rlo,\rshort,a,\rEES)}(\hs) &=& \sum_{J=1}^N F_{IJ}^{(\rlo,\rshort,a)}\sum_{\bk} \Mp(\bu_{J}-\hat \bs) \delta_{\hat \bs,\bl_J-\bk} \hsinpEES \\
F_I^{(W,\rP\ 1,\rlo,\rshort,b,\rEES)}(\hs)&=& \sum_J Z_{IJ}^{(\rS,\rEES)}\left[\sum_{\hspJp}^\pzp \phi_J^{(\rP\ 1,\rlo,\rshort,\rEES)}(\hs') \delta_{\hs,\hs'}\right]\hsinpEES \nonumber\\
\\
F_I^{(W,\rP\ 2,\rlo,\rshort,\rEES)}(\hs) &=& \sum_J  Z_{IJ}^{(\rS,\rEES)} \phi_J^{(\rP\ 2,\rlo,\rshort)} \sum_{\bk}  \Mp(\bu_{J}-\hat \bs)\delta_{\hat \bs,\bl_J-\bk} \hsinpEES\nonumber
\\ 
...
\begin{split}
\frac{\p E}{\p \psigsc} &=  \sum_{i=1}^{N^{(\rgr,n,-)}}p_i^{(n)}(\bg)\FFTni \left[A_i^{(n)}(\hs),\hGc\right] \\
&+ \sum_{j=1}^{N^{(\rgr,\Psi,-,\rEES)}}p_j^{(\Psi,\rEES)}(\bg)\FFTpiEES \left[A_j^{(\Psi,\rEES)}(\hs),\hGc\right] \\
\end{split}
\end{equation}
...
A_1^{(\Psi,\rEES)}(\hs) &\hspace*{1.8in}\hsinpEES \\
p_2^{(\Psi,\rEES)}(\bg) &= \Dpgc \frac{1}{\sqrt{V}}\hspace*{1.0in} |\bg| \leq \hGc \\
A_2^{(\Psi,\rEES)}(\hs) &\hspace*{1.8in} \hsinpEES \\
\end{split}
\end{equation}

We provide the definitions of the $A_i^{(n)}(\hs)$ and the $A_j^{(\Psi,\rEES)}(\hs)$ below with the other terms already defined above.\\
...
\subsection{The $\{p_1^{(\Psi,\rEES)}(\bg), A_1^{(\Psi,\rEES)}(\hs)\}$ terms}
...
In order to derive $Z_{IJ}^{(W,\rgr,\rcore,\rEES)}$, we start from the ``$A_1^{(\Psi,\rEES)}(\hs) $'' terms from the kinetic energy:
\begin{equation}
\begin{split}   
...
In order to derive $Z_{IJ}^{(W,\rgr,\rP\ 1,\rshort,\rEES)}$, we start from the ``$A_1^{(\Psi,\rEES)}(\hs) $'' terms from core short local e-ion, core exchange correlation, short smooth-core Hartree, short core-core Hartree associated with the ($\rP$ 1) densities:
...
\item \ReE $\phi_J^{(\rcore,\rS,\rH,\rshort,\chi,D,\rEES)}(\br_f)$ in Eq.\eqref{phi:fSchiEES}.
...
In order to derive $Z_{IJ}^{(W,\rgr,\rP\ 2,\rshort,\rEES)}$, we start from the ``$A_1^{(\Psi,\rEES)}(\hs) $'' terms from short local e-ion, PAW core exchange correlation, short smooth-core Hartree, short core-core Hartree associated with the ($\rP$ 2) densities:
...
In order to derive $Z_{IJ}^{(W,\rgr,\rP\ 1,\rlong,\rEES)}$, we start from the ``$A_1^{(\Psi,\rEES)}(\hs) $'' terms from long local e-ion, long Hartree associated with the ($\rP$ 1) densities:
...
In order to derive $Z_{IJ}^{(W,\rgr,\rP\ 2,\rlong,\rEES)}$, we start from the ``$A_1^{(\Psi,\rEES)}(\hs) $'' terms from long local e-ion, long Hartree associated with the ($\rP$ 2) densities:
...
\subsection{The $\{p_2^{(\Psi,\rEES)}(\bg), A_2^{(\Psi,\rEES)}(\hs)\}$ terms}
...
\subsection{The $\{p_1^{(n)}(\bg), A_1^{(n)}(\hs)\}$ terms}
...
A_1^{(n)}(\hs) &= \Psi_{I}^{(\rS)}(\hat \bs) \phi^{(W,\rgr,n)}(\hat \bs) \hsinn \\ 
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
\frac{\p E}{\p \psigsc} &=  \frac{1}{\sqrt{V}}\FFTni \left[A_1^{(n)}(\hs),\hGc\right] \\
&+ \Dpgc \tilde p^{(\rS)*}(\bg) \frac{1}{\sqrt{V}} \FFTpiEES \left[A_1^{(\Psi,\rEES)}(\hs),\hGc\right] \\
&+ \Dpgc \frac{1}{\sqrt{V}} \FFTpiEES \left[A_2^{(\Psi,\rEES)}(\hs),\hGc\right] \\
\end{split}
\end{equation}

where
\begin{enumerate}
\item \ReWG $\phi^{(W,\rgr,n)}(\hs)$ is defined in Eq.\eqref{gr:A1ns}.
\item \ReWG $ A_1^{(\Psi,\rEES)}(\hs)$ is defined in  Eq.\eqref{gr:A1pEES}
\item \ReWG $A_2^{(\Psi,\rEES)}(\hs) $ is defined in Eq.\eqref{gr:groupedFDPsi}
\end{enumerate}
...
\ReW \phi_J^{(\rP\ l,\rcore,\rH,\rshort,\rself,\rEES)}(\br_f) & =& \frac{e^2}{2}\sum_{f'} w_f' n_J^{(\rP\ l,\rEES)}(\br_{f}')\phi_{JK}^{(\rcore,\rcore,\rH,\rshort,\rself)}(\br_f,\br_{f'}) \\
...
\underline{Core-tot Hartree long energy:}
\begin{eqnarray}
F_{J\beta}^{(R,\rP\ 1,\rtot,\rH,\rlong,1,a,\rEES)}(\hs) &=& \sum_f w_f \Delta p(\br_f) \phi_J^{(\rcore,\rtot,\rH,\rlong,\chi,D,\rA,\rEES)}(\br_f) \sum_{\bk} \frac{\p \Mp(\bu_{Jf}-\hat \bs)}{\p \RJb} \delta_{\hat \bs,\bl_{Jf}-\bk} \nonumber\\ 
&& \hspace*{2.5in} \hsinJp 
\end{eqnarray}
...
\frac{\p E^{(\rP\ 1,\rtot,\rH,\rlong,a,\rEES)}}{\p \RJb}&=&\sum_{\hsJp}^{\pzp} \left(\Psi_J^{(\rS,Z,D)}(\hat \bs)+ \rcc\right) F_{J\beta}^{(R,\rP\ 1,\rtot,\rH,\rlong,1,a,\rEES)}(\hs) \\
...
We will label these terms when they appear as $\ReRG$.\\
...
&+& \left. P^{(\rKE)}\hspace*{1in} \frac{\p Z_{J}^{(\rS,2,\rEES)}}{\p \RJb} \fr \eqref{gr:dEKEdR} \right. \\
&+& \left. \phi_J^{(\rP\ 2,\rlo,\rshort)}\hspace*{0.4in} \frac{\p Z_{J}^{(\rS,2,\rEES)}}{\p \RJb} \fr \eqref{gr:dElocshosrt2AdR} \right. \\
&+& \left. \phi_J^{(\rP\ 2,\rxc,\rA,\rEES)}\hspace*{0.4in}\frac{\p Z_{J}^{(\rS,2,\rEES)}}{\p \RJb} \fr \eqref{gr:dExc2AdR} \right. \\
&+& \left. \phi_J^{(\rP\ 2,\rS,\rH,\rshort,\rA,\rEES)}\frac{\p Z_{J}^{(\rS,2,\rEES)}}{\p \RJb} \fr \eqref{gr:dEHshortS2AdR} \right. \\
&+& \left. \phi_J^{(\rP\ 2,\rcore,\rH,\rshort,\rEES)}\frac{\p Z_{J}^{(\rS,2,\rEES)}}{\p \RJb} \fr \eqref{gr:dEHshortcore2AdR} \right. \\
\left. \right\} &&
\end{eqnarray}
...
&&\left. \left(\Psi_J^{(\rS,Z,D)}(\hat \bs)+ \rcc\right)\hspace*{0.4in} F_{J\beta}^{(R,\rP\ 1,\rlo,\rshort,\rA,a,\rEES)}(\hs) \fr \eqref{gr:dElocshosrt1AadR} \right. \\
&+& \left.  \left(\Psi_J^{(\rS,Z,D)}(\hat \bs)+ \rcc \right)\hspace*{0.4in} F_{J\beta}^{(R,\rP\ 1,\rxc,\rA,a,\rEES)}(\hs) \fr \eqref{gr:dExc1AadR}\right.\\
&+& \left.  \left(\Psi_J^{(\rS,Z,D)}(\hat \bs)+ \rcc\right)\hspace*{0.4in} F_{J\beta}^{(R,\rP\ 1,\rS,\rH,\rshort,\rA,a,\rEES)}(\hs) \fr \eqref{gr:dEHshortS1AadR}\right.\\
...
&+& \left.  \left(\Psi_J^{(\rS,Z,D)}(\hat \bs)+ \rcc\right) \hspace*{0.4in}F_{J\beta}^{(R,\rP\ 1,\rtot,\rH,\rlong,1,a,\rEES)}(\hs) \fr \eqref{gr:dEHlong1adR}\right.\\
&+& \left.\phi_J^{(\rP\ 1,\rlo,\rshort,\rEES)}(\hs) \hspace*{0.21in}F_{J\beta}^{(R,\rS,Z,\Psi,\rEES)}(\hs) \fr \eqref{gr:dElocshosrt1AbdR}\right.\\
&+& \left.  \phi_J^{(\rP\ 1,\rxc,\rA,\rEES)}(\hs)\hspace*{0.4in}F_{J\beta}^{(R,\rS,Z,\Psi,\rEES)}(\hs) \fr \eqref{gr:dExc1AbdR}\right.\\
&+& \left.  \phi_J^{(\rP\ 1,\rS,\rH,\rshort,\rA,a,\rEES)}(\hs)F_{J\beta}^{(R,\rS,Z,\Psi,\rEES)}(\hs) \fr \eqref{gr:dEHshortS1AbdR}\right.\\
&+& \left.  \phi_J^{(\rP\ 1,\rcore,\rH,\rshort,\rEES)}(\hs)\hspace*{0.05in}F_{J\beta}^{(R,\rS,Z,\Psi,\rEES)}(\hs) \fr \eqref{gr:dEHshortcore1AbdR}\right.\\
&+& \left.  \phi_J^{(\rP\ 1,\rtot,\rH,\rlong,\rA,\rEES)}(\hs)\hspace*{0.05in}F_{J\beta}^{(R,\rS,Z,\Psi,\rEES)}(\hs)  \fr \eqref{gr:dEHlong1bdR} \right. \\
\left. \right\} &&
\end{eqnarray}
...
&+ F_{J\beta}^{(R,\rP\ 1,\rS,\rH,\rshort,\rA,a,\rEES)}(\hs)
+ F_{J\beta}^{(R,\rP\ 1,\rcore,\rH,\rshort,\rA,a,\rEES)}(\hs)\\
&+ F_{J\beta}^{(R,\rP\ 1,\rtot,\rH,\rlong,1,a,\rEES)}(\hs)\\
\end{split}
\end{equation}
and
\begin{equation}
...
&= \phi_J^{(\rP\ 1,\rlo,\rshort,\rEES)}(\hs)
+ \phi_J^{(\rP\ 1,\rxc,\rA,\rEES)}(\hs) \\
&+ \phi_J^{(\rP\ 1,\rS,\rH,\rshort,\rA,a,\rEES)}(\hs)
+  \phi_J^{(\rP\ 1,\rcore,\rH,\rshort,\rEES)}(\hs)\\
&+ \phi_J^{(\rP\ 1,\rtot,\rH,\rlong,\rA,\rEES)}(\hs)\\
\end{split}
\end{equation}
...
&&\left. Q_J \left[ \sum_{\bk} \frac{\p M_p^{(3,n)}(\bu_{J}-\hat \bs)}{\p \RJb} \phi^{(R,\rS,\rlo,\rcomb,\chi,D)}(\hs)\right]\delta_{\hs,\bl_J-\bk} \fr \eqref{gr:dElocSEESdR} \right. \\
&+& \left.  Q_J \sum_{\bk} \phi^{(R,\rcore,\rlo,\rlong,\chi,D,\rB,\rEES)}(\hs) \frac{\p \Mn (\bu_{J}-\hat \bs)}{\p \RJb} \delta_{\hs,\bl_J-\bk}  \fr \eqref{gr:dEloclongBdR} \right. \\
\left. \right\} &&
\end{eqnarray}
...
&&\left. n^{(\rS,D)}(\hat \bs) F_{J\beta}^{(R,\rcore,\rxc,\rB,\rEES)}(\hs) \fr \eqref{gr:dExcBdR} \right. \\
&+& \left.  \phi^{(\rcore,\rS,\rH,\rshort,\chi,D)}(\hs)F_{J\beta}^{(R,\rcore,\rS,\rH,\rshort,\rB,\rEES)}(\hs) \fr \eqref{gr:dEHshortSBdR} \right.\\
&+& \left. \phi^{(\rcore,\rlo,\rlong,\chi,S,D,\rEES)}(\hs)  F_{J\beta}^{(R,\rP\ 1,\rEES)}(\hs) \fr \eqref{gr:dEloclong1AcdR}  \right. \\
&+& \left.  \phi^{(\rcore,\rtot,\rH,\rlong,\chi,D,\rA,\rEES)}(\hs) F_{J\beta}^{(R,\rP\ 1,\rEES)}(\hs) \fr \eqref{gr:dEHlong1cdR} \right.\\
&+& \left.  \phi^{(\rcore,\rlo,\rlong,\chi,S,D,\rEES)}(\hs) \left[F_{J\beta}^{(R,\rS,Z,2,\rEES)}(\hs)+ F_{J\beta}^{(R,\rP\ 2,\rEES)}(\hs)\right] \fr \eqref{gr:dEloclong2AdR}\right.\\
...
&+ \left.   \phi^{(\rcore,\rS,\rH,\rshort,\chi,D)}(\hs)F_{J\beta}^{(R,\rcore,\rS,\rH,\rshort,\rB,\rEES)}(\hs) \right.\\
&+ \left.  \phi^{(R,\rgr,\rlong,\rEES)}(\hs) F_{J\beta}^{(R,\rgr,\rEES)}(\hs) \right\} \\
\end{split}
\end{equation}
...
&&\left. - n_{J}^{(\rcore,\rEES)}(\br_f) \frac{\p \phi_J^{(\rcore,\rlo,\rshort,\rNN)}(\br_f)}{\p \RJb} \fr \eqref{gr:dElocshortBdR} \right. \\
...
\begin{equation}
\begin{split}
\frac{\p E}{\p \psigsc} &= \sum_i p_i^{(n)}(\bg) \mathrm{FFT}^{(n)} \left[A_i^{(n)}(\hs),\hGc\right]\\
...
&+ \sum_j \sum_{\hsJp}^{\pzp} A_j^{(\Psi,\rEES)}(\hs)\\
